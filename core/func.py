        ...
        if action_list_tmp:
            if not role.ai:
                ...
                while action_invalid:
                    ...
                    try:
                        action_true = action_list_tmp[int(input_string) - 1]
                        # print(action_true)
                    except:
                        pass
                    if not(action_true is None):
                        # print("YES")
                        action_invalid = False
                # ---------------------------------------
            else:
                action_true = random.choice(action_list_tmp)
        else:
            print("本回合{}无可用行动".format(role.nickname))

        # 设置
        # -----------------------------------------------------------------
        if action_true is not None:
            action_string = action_true.code
...
def main_round(input_pool, num_round=0):
    """
    主结算函数
    ...
    print("--------------第{0}回合-------------".format(num_round + 1))
    ...
    while not is_order_phase_end:
        ...
        for role in pool[::-1]:
            ...
            action = role.get_role_action()
            # 判断当前遍历的角色能否在这个阶段行动
            if not (action is None) and action.order_phase:
                # 如果有行动, 且行动有起始阶段, 继续
