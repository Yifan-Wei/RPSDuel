...
class RoleBase(object):
    ...
    def __init__(self):
        ...
        # 角色默认状态
        self.status_default = {"basic": {}, "last_basic": {}, "buff": {}, "round": {}, "last_round": {}}
        # 角色当前状态
    ...
    def init_basic_status(self):
        ...
        basic_list = ["ROLE_HP", "ROLE_MP", "ROLE_DUR", "ROLE_CORE", "ROLE_STR", "ROLE_INT", "ROLE_SPD", "ROLE_ARM",
                      "ROLE_MAX_HP", "ROLE_MAX_MP", "ROLE_MAX_DUR", "ROLE_MAX_CORE", "ROLE_MAX_ARM"]
    ...
    def print_basic_status(self):
        ...
        string = "HP: {0}/{1}\nDUR: {2}/{3}\nSTR: {4}".format(self.get_role_hp(), self.get_role_hp(mode="max"),
                                                              self.get_role_dur(), self.get_role_dur(mode="max"),
                                                              self.get_role_str())
        print(string)
        print("#----------------------------------------")
    ...
    def init_buff_status(self):
        ...
        buff_list = ["DEFENDING", "DEFENDING_BEATBACK", "EVADING", "EVADING_BEATBACK", "PIERCING", "PRECISE",
                     "CRITICAL", "HPLOSS", "TOUGH", "REGENERATION", "INVISIBLE", "BLANKING"]
        # 通用增益
        # 按顺序: 格挡, 格挡反击, 闪避, 闪避反击, 穿甲, 必中, 暴击, 流失, 坚韧, 再生, 隐匿, 消隐

        debuff_list = ["BLIND", "WEAK", "WOUNDED", "BURNT", "POISONED", "CURSED", "ILLUSORY", "DISARMED", "SILENCED",
                       "STUNNED"]
        # 通用减益
    ...
    def init_round_status(self):
        ...
        # 记录耐力消耗值
        self.status_default["round"]["ROUND_DUR_COST"] = 0
        self.status_default["last_round"]["ROUND_DUR_COST"] = 0
        # 记录回合开始时的耐力
    ...
    def get_role_str(self):
        ...
        """
        # 当前血量和生命上限
        string = self.status_current["basic"]["ROLE_STR"]
        return string
    ...
    def set_role_str(self, value):
        ...
        if value >= 0:
            ...
            self.status_current["basic"]["ROLE_STR"] = math.ceil(string + value)
        elif value < 0:
            # 削弱力量,力量可以为负数,但结算伤害的时候伤害不能
            self.status_current["basic"]["ROLE_DUR"] = math.ceil(string + value)

        print("{0}的力量从{1}变更为{2}".format(self.nickname, string, self.status_current["basic"]["ROLE_STR"]))
    ...
    def set_role_hp(self, value, mode="now", bl_recover=False):
        ...
        print("{4}的生命值从{0}/{1}变更为{2}/{3}".format(hp, max_hp, self.status_current["basic"]["ROLE_HP"],
                                                 self.status_current["basic"]["ROLE_MAX_HP"], self.nickname))
    ...
    def set_role_dur(self, value, mode="now", bl_recover=False):
        ...
        if mode == "now":
            ...
            if value >= 0:
                ...
                # 如果当前耐力大于上限, 则降低到上限
                if dur > self.status_current["basic"]["ROLE_MAX_DUR"]:
                    self.status_current["basic"]["ROLE_DUR"] = self.status_current["basic"]["ROLE_MAX_DUR"]

        print("{0}的耐力值从{1}/{2}变更为{3}/{4}".format(self.nickname, dur, max_dur, self.status_current["basic"]["ROLE_DUR"],
                                                 self.status_current["basic"]["ROLE_MAX_DUR"]))
    ...
    def get_round_dur_cost(self):
        # -----------------------
        # 获取角色当前回合的耐力消耗数, 省事的小函数
        # -----------------------
        if "ROUND_DUR_COST" in self.status_current["round"].keys():
            return self.status_current["round"]["ROUND_DUR_COST"]
        return 0
    ...
    def get_buff_status(self, buff_string):
        ...
        if buff_string in self.status_current["buff"]:
            ...
            buff_list = self.status_current["buff"][buff_string]
            return buff_list
        else:
            return

    def add_buff_status(self, buff_string, add_layer, add_buff=None, add_mode="layer"):
        """
        ...
        if buff_string in self.status_current["buff"]:
            ...
            buff_list = []
            self.status_current["buff"][buff_string] = buff_list

        if add_mode != "anyway":
            # 模式不是独立叠加
            for item in buff_list[::-1]:
                ...
                if is_buff_the_same(dut_buff=item, cmp_buff=add_buff):
                    ...
                    if add_mode == "layer":
                        ...
                        print("{3} - {0} BUFF 从{1}层 叠加到 {2}层".format(buff_string, layer, item["BUFF_LAYER"],
                                                                     self.nickname))
                    elif add_mode == "refresh":
                        # 刷新, 重置为两者中的较大值
                        item["BUFF_LAYER"] = max(layer, add_layer)
                        print("{3} - {0} BUFF 从{1}层 刷新到 {2}层".format(buff_string, layer, item["BUFF_LAYER"],
                                                                     self.nickname))
        ...
        buff_list.append(add_buff)
        print("{2} - 新增 {0} BUFF: {1}层".format(buff_string, add_buff["BUFF_LAYER"], self.nickname))
        return 1

    def rm_buff_status(self, buff_string, rm_layer, pool, rm_buff=None, rm_mode="SPECIFIC", bill_mode="BILL"):
        ...
        for item in buff_list[::-1]:
            ...
            if rm_mode == "SPECIFIC":
                ...
                if is_buff_the_same(dut_buff=item, cmp_buff=rm_buff):
                    ...
                    rm_buff_status_from_dict(buff_string=buff_string, buff_list=buff_list, buff=item, rm_layer=rm_layer,
                                             pool=pool, bill_mode=bill_mode)
                    # 不跳出,记录当前移除
                    remover = True

            elif rm_mode == "ANYWAY":
                # 强力清扫模式,不讨论是否一致,直接移除
                rm_buff_status_from_dict(buff_string=buff_string, buff_list=list, buff=item, rm_layer=rm_layer,
                                         pool=pool, bill_mode=bill_mode)
                # 不跳出,记录当前移除
                remover = True
    ...
    def iter_buff_status(self, pool):
        ...
        # 首先搞清楚里面大概是个这样的状态: 第一层是字典, 所有buff的字段在这里 第二层是列表, 同一个类型的buff可能有不同来源,用列表来排列 第三层是字典, 字典键带有buff来源, buff对象,
        # buff正面标识符, buff负面标识符, buff脱落标识符, buff层数, buff数值, buff优先级, buff预留字段 关键字是 BUFF_SOURCE=[], BUFF_TARGET=[],
        # BUFF_IS_POSITIVE=0/1, BUFF_IS_NEGATIVE=0/1, BUFF_IS_SHEDDING=0/1, BUFF_LAYER=INTEGER, BUFF_VALUE=FLOAT,
        # BUFF_PRIORITY=INTEGER, BUFF_PRESERVE=None 在叠加buff的是时候,只有buff来源、对象一致的buff才会叠加/重置
            ...
            if len(buff_value_list) > 0:
                ...
                for value_list_dict in buff_value_list[::-1]:
                    ...
                    if value_list_dict["BUFF_IS_SHEDDING"] == 1:
                        ...
                        rm_buff_status_from_dict(buff_string=buff_key, buff_list=buff_value_list, buff=value_list_dict,
                                                 rm_layer=1, pool=pool, billmode="BILL")
    ...
    def is_affected(self):
        ...
        需要满足 不是消隐状态或者隐匿状态
        """
        if self.is_buffed("BLANKING") or self.is_buffed("INVISIBLE"):
            return False
        return True
    ...
    def js_print(self):
        ...
        dict_print = {"id": self.id, "code": self.code, "job": self.job, "stream": self.stream,
                      "status_default": self.status_default, "status_current": self.status_current}
