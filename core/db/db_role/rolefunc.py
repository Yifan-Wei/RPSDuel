...
def get_role_from_id(input_id, pool):
    # 遍历池子里的结构体
    for member in pool:
        if member.id == input_id:
            return member
    # 找不到返回空
    return None
...
def is_buff_the_same(dut_buff=None, cmp_buff=None):
    """
    # 第三层是字典, 字典键带有buff来源, buff对象, buff正面标识符, buff负面标识符, buff脱落标识符, buff层数, buff最大层数, buff次数, buff数值, buff优先级,
    buff预留字段 # 关键字是 BUFF_SOURCE=[], BUFF_TARGET=[], BUFF_IS_POSITIVE=0/1, BUFF_IS_NEGATIVE=0/1, BUFF_IS_SHEDDING=0/1,
    BUFF_IS_DISPELLABLE =0/1, BUFF_IS_VISIBLE=0/1, BUFF_LAYER=INTEGER, BUFF_MAX_LAYER = INTEGER, BUFF_COUNT=INTEGER,
    BUFF_VALUE=FLOAT, BUFF_PRIORITY=INTEGER, BUFF_PRESERVE=None # 比较来源、对象、正负面、衰减、可驱散、可见、最大层数、值、优先级
    """
    # 先把buff可能空置的地方填满
    if dut_buff is None:
        dut_buff = {}
...
def rm_buff_status_from_dict(buff_string, buff_list, buff, rm_layer, pool, billmode="BILL"):
    ...
    while iter_layer > 0:
        ...
        if billmode == "BILL":
            ...
            # 按顺序是 灼烧 中毒 诅咒
            if buff_string == "BURNT":
                # 灼烧时, 所有目标生命值-5
                for target in target_list:
                    target.set_role_hp(-5.0)

            elif buff_string == "POISONED":
                # 中毒时, 所有目标失去层数生命
                for target in target_list:
                    target.set_role_hp(-1.0 * layer)
            ...
            if buff_string == "FEATURE_WARRIOR_MASTER":
                ...
                for target in target_list:
                    if target.is_confronted():
                        ...
                        target.set_role_dur(target.get_round_dur_cost())
...
def is_qualified_to_act(role, condition=None):
    """
    role: 判断主体
    condition: 判断条件, 是个字典
    return True/False
...
def trig_action_cost(role, condition=None):
    # 所有条件遍历完成之后, 开始处理结算条件
    if condition is None:
        condition = {}
    for key, value in condition.items():
        ...
        if key == "COST_DUR":
            ...
            # 回合指示器: DUR消耗情况
            role.status_current["round"]["ROUND_DUR_COST"] += value
        # --------------------------------------


def exert_effect_to_role(role, target, spell_effect=None):
    """
    施加额外效果的函数
    ...
    for key, value in spell_effect.items():
        ...
        if "REC_" in key:
            ...
            if key == "EXERT_BUFF_POISONED":
                ...
                target.add_buff_status(buff_string="POISONED", add_layer=value, add_buff=buff, add_mode="layer")
                continue
            ...
            if key == "FEATURE_WARRIOR_MASTER":
                ...
                buff["BUFF_IS_POSITIVE"] = 1
                buff["BUFF_IS_DISPELLABLE"] = 0
                target.add_buff_status(buff_string="FEATURE_WARRIOR_MASTER", add_layer=value, add_buff=buff,
                                       add_mode="layer")
                continue
            ...
            if key == "FEATURE_WARRIOR_SAMURAI":
                ...
                buff["BUFF_IS_POSITIVE"] = 1
                buff["BUFF_IS_DISPELLABLE"] = 0
                target.add_buff_status(buff_string="FEATURE_WARRIOR_SAMURAI", add_layer=value, add_buff=buff,
                                       add_mode="layer")
                continue
...
def exert_effect(role, step_content=None):
    """
...
def trig_harm(damager, victim, value, extra=None):
    """
    在这里触发伤害相关的行动
    可能不是最好的实现方法
    半成品
...
def give_harm(role, target, value, extra=None):
    """
    ...
    # 被攻击目标
    # 坚韧*0.5 重创*1.5
    if target.is_buffed("TOUGH"):
        target_multi_index *= 0.5
        string += "坚韧的"
    if target.is_buffed("WOUNDED"):
...
def cal_damage(role, content=None, order=0):
    """
    ...
    if content is None:
        content = {}
    damage_result = {"HAS_CONFRONTED": False, "HAS_HIT": False, "HAS_HARM": False, "BEEN_MISSED": False,
                     "BEEN_DEFENDED": False, "BEEN_BLOCKED": False}
    # 顺序包含以下字段，先初始化:
    ...
    for target in target_list:
        ...
        if target.is_affected() or role.is_unavoidable():
            ...
            if is_harm_hit:
                ...
                harm_value_after_defending, is_harm_defended = get_harm_result(role=role, target=target,
                                                                               value=harm_value)
                # 判断是否伤害对方 (伤害值大于0)
        ...
        if is_harm_hit:
            ...
            if is_harm_hurt:
                ...
                give_harm(role=role, target=target, value=harm_value_after_defending,
                          extra={"is_confronting": is_confronting})
                # 结算技能的伤害后效果IF_HARM
